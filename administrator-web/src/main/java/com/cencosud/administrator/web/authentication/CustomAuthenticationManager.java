 ...
 */
package com.cencosud.administrator.web.authentication;
...
import com.cencosud.administrator.core.SystemLogServices;
import com.cencosud.administrator.core.UserServices;
import com.cencosud.administrator.core.enums.LogEnum;
...
import com.cencosud.administrator.web.utils.ValidacionLoginLdapServicio;
import com.cencosud.administrator.web.utils.ValidacionServicioUtils;
...
public class CustomAuthenticationManager implements AuthenticationManager {
	...
	public Authentication authenticate(Authentication auth)
		...
		if ("admin".equals(auth.getName())
			...
			if (currentUser != null) {
				if (currentUser.getStatus() != StatusEnum.BLOCKED_USER
					...
					try {
						/*
						equals = ValidacionServicioUtils.isValidAddressService(
								String.valueOf(auth.getName()),
								String.valueOf(auth.getCredentials()));
								*/
						
						equals = ValidacionLoginLdapServicio.ValidarloginLdapService(
								String.valueOf(auth.getName()),
								String.valueOf(auth.getCredentials()));
						log.info("user: equals " + equals);
					} catch (Exception e) {
						log.error("Password invalida");
						throw new BadCredentialsException("Password invalida");
					}
		...
		String res = ValidacionServicioUtils.isValidAddResponseService(
				String.valueOf(auth.getName()),
				String.valueOf(auth.getCredentials()));
		/*
		String res = ValidacionLoginLdapServicio.ValidarloginInfoLdapService(
				String.valueOf(auth.getName()),
				String.valueOf(auth.getCredentials()));
		*/
		//log.info(" res =============== "+res);
		...
		if ("Admin".equals(response)) {
			...
			PadrinoDTO padrino = padrinoServicesImpl.getPadrino(response);
			//log.info(" Nombre  = "  + padrino.getDescription());
			if (padrino != null) {
				currentUser.setName(padrino.getDescription());
				log.info(" Padrino  Ok " );
			} else {
				log.info(" Padrino No Ok " );
				PadrinoDTO padrino2 = new PadrinoDTO();
				//padrino.setId(new Long(1));
				padrino2.setDescription(response);
				TiendaDTO tienda = new TiendaDTO(new Long(1));
				padrino2.setTienda(tienda);
				
				
				padrinoServicesImpl.addPadrino(padrino2);
				currentUser.setName(response);
			}
		}
