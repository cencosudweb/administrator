        ...
        <div class="container-fluid">
	        <div class="row-fluid">
				<div class="btn-toolbar">
					...
					%>
						<a class="btn" href="reporteCuadraturaVentaEnVerdePrint?exportToExcel=YES"><i class="icon-book"></i> Exportar a Excel</a>
					<%
						} else {
					%>
						<a target="_blank" class="btn" href="reporteCuadraturaVentaEnVerdePrint?exportToExcel=YES&posted=<%if(request.getParameter("posted") != null){%><%=request.getParameter("posted")%><%}else{%><%=""%><% } %>&dateInicio=<% if (request.getParameter("dateInicio") != null) { %><% try { SimpleDateFormat formatoFecha = new SimpleDateFormat("dd/MM/yyyy", Locale.getDefault()); formatoFecha.setLenient(false); formatoFecha.parse(request.getParameter("dateInicio"));%><%=request.getParameter("dateInicio").toString().trim()%><% } catch(ParseException  e) {%><%=currentDate%><% } %><%  } else {%><%=currentDate %> <% } %>&estado=<%if(request.getParameter("estado") != null){%><%=request.getParameter("estado")%><%}else{%><%="-1"%><% } %>&solicitud_original=<%if(request.getParameter("solicitud_original") != null){%><%=request.getParameter("solicitud_original")%><%}else{%><%=""%><% } %>&orden_distribucion=<%if(request.getParameter("orden_distribucion") != null){%><%=request.getParameter("orden_distribucion")%><%}else{%><%=""%><% } %>"><i class="icon-book"></i> Exportar a Excel</a>
				...
				<div class="panel panel-default">
					<div class="panel-body">
						<div class="btn-toolbar">
							<form class="form-inline" id="f_all_user" method="get" action="reporteCuadraturaVentaEnVerde">
								...
								<select name="estado" id="estado" class="form-control" style="width:150px;">
					              ...
					              <c:forEach items="${tipoestadovtavs}" var="listTipoEstado" varStatus="status">
					            ...
					            <label>&nbsp;&nbsp;DO EOM&nbsp;</label>
					            &nbsp;&nbsp;
								<input class="form-control span12" style="width:150px;" placeholder="Buscar DO EOM..." name="orden_distribucion" id="orden_distribucion" type="text" value="<%if(request.getParameter("orden_distribucion") != null){%><%=request.getParameter("orden_distribucion")%><%}else{%><%=""%><% } %>">
					            <label>&nbsp;Solicitud EOM&nbsp;</label>
					            &nbsp;&nbsp;
       							<input class="form-control span12" style="width:150px;" placeholder="Buscar Solicitud EOM..." name="solicitud_original" id="solicitud_original" type="text" value="<%if(request.getParameter("solicitud_original") != null){%><%=request.getParameter("solicitud_original")%><%}else{%><%=""%><% } %>">
				...
				<span class="label label-warning" style=" background-color:#99994d">Cantidad OC</span>
					...
					<c:choose>
					 ...
					 <table class="table table-bordered table-striped">
							...
							try{
								...
								if ("1".equals(request.getParameter("estado"))) {
									...
									SQLFecha = 	" SELECT " +
											...
											" (select count(*)   from CUADRATURA_OC_VTAV_COMP  where 1 = 1 and FECHA_INGRESO >= '"+starttime+"' AND FECHA_INGRESO <= '"+starttime+"' group by RELNUM,POMUMB,INUMBR,RELBL5 Having sum(CANTIDAD_COMP1)< max(CANTIDAD_COMP2) ) as cantidadOC " +
					  ...
					  <tr>
						...
						<td>
						<span class="label label-warning" style=" background-color:#99994d;">
						<% 
						
						if (rsFecha.getInt("cantidadOC")>0) {
						%>
						<a href="detalleCuadraturaVentaEnVerdeCantidad?fecha=<%=starttime%>"  target="_blank" style="color:#FFFFFF"><% out.println(formatea.format(rsFecha.getInt("cantidadOC"))); %></a>
						<% 
							
						} else {
							out.println("No hay Diferencias");
						}
