				...
				<div class="well" style="overflow:scroll; height:600px">
				      ...
				      <%
							...
							try{
								...
								SQLFecha = "select ID, SOLICITUDORIGINAL, N_ORDEN_DISTRIBU, SOLICITUD, FECHA_CREACION_ORDEN, ORIGEN, CANCELADO  from CUADRATURA_OC_VTAV_EOM A  WHERE N_Orden_Distribu NOT IN (SELECT NUMDO FROM CUADRATURA_OC_VTAV_JDA B WHERE B.NUMDO = A.n_orden_distribu and B.Pofece >= '"+starttime+"' and B.Pofece <= '"+starttime+"') and A.fecha_creacion_orden >= '"+starttime2+" 00:00:00' and A.fecha_creacion_orden <= '"+starttime2+" 23:59:59' ";
//SQLFecha = "select ID, SOLICITUDORIGINAL, N_ORDEN_DISTRIBU, SOLICITUD, FECHA_CREACION_ORDEN, ORIGEN, CANCELADO, CASE WHEN (SELECT CVV_JDA.NUMDO FROM CUADRATURA_OC_VTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '20170418' AND CVV_JDA.Pofece <= '20170418' AND CVV_JDA.NUMDO = A.n_orden_distribu) IS NOT NULL THEN 'Encontrado' ELSE 'No Encontrado' END    as ENCONTRADOENJDA, CASE WHEN (SELECT CVV_JDA.NUMDO FROM CUADRATURA_OC_VTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = A.n_orden_distribu) IS NOT NULL THEN (SELECT CVV_JDA.NUMDO FROM CUADRATURA_OC_VTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = A.n_orden_distribu) ELSE 'Numero solicitud Jda No Encontrado' END    as NUMEROSOLICITUDJDA, CASE WHEN (SELECT CVV_JDA.EX14ERROR FROM CUADRATURA_OC_VTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = A.n_orden_distribu) IS NOT NULL THEN (SELECT CVV_JDA.EX14ERROR FROM CUADRATURA_OC_VTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = A.n_orden_distribu) ELSE 'Mensaje de Error Jda No Encontrado'  END    as MENSAJEERRORJDA, CASE WHEN (SELECT CVV_JDA.Pofece FROM CUADRATURA_OC_VTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = A.n_orden_distribu) IS NOT NULL THEN (SELECT CVV_JDA.Pofece FROM CUADRATURA_OC_VTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = A.n_orden_distribu) ELSE 'Fecha Jda No Encontrado'  END as POFECEJDA  from CUADRATURA_OC_VTAV_EOM A  WHERE 1 = 1  and A.fecha_creacion_orden >= TO_DATE('"+starttime2+"','DD/MM/YY')||' 00:00:00' and A.fecha_creacion_orden <= TO_DATE('"+starttime2+"','DD/MM/YY')||' 23:59:59' AND A.N_ORDEN_DISTRIBU = '"+request.getParameter("N_Orden_Distribu")+"'";
							...
							%>
					  <tr>
					  	<td nowrap="nowrap"><%=rsFecha.getString("ID") %></td>
						<td nowrap="nowrap"><%=rsFecha.getString("SOLICITUDORIGINAL") %></td>
						<td nowrap="nowrap"><%=rsFecha.getString("N_ORDEN_DISTRIBU") %></td>
						<td nowrap="nowrap"><%=rsFecha.getString("SOLICITUD") %></td>
						<td nowrap="nowrap"><%=rsFecha.getString("FECHA_CREACION_ORDEN") %></td>
						<td nowrap="nowrap"><%=rsFecha.getString("ORIGEN") %></td>
						<td nowrap="nowrap"><%=rsFecha.getString("CANCELADO") %></td>
						<!--  
						<td>
						<a target="_blank" href="informacionEom?dateInicio=<% if (request.getParameter("dateInicio") != null) { %><% try { SimpleDateFormat formatoFecha = new SimpleDateFormat("dd/MM/yyyy", Locale.getDefault()); formatoFecha.setLenient(false); formatoFecha.parse(request.getParameter("dateInicio"));%><%=request.getParameter("dateInicio").toString().trim()%><% } catch(ParseException  e) {%><%=currentDate%><% } %><%  } else {%><%=currentDate %> <% } %>" title="POP UP" ><i class="icon-signin"></i>No registros desde EOM en JDA</a><br />
						<a target="_blank" href="informacionJda?dateInicio=<% if (request.getParameter("dateInicio") != null) { %><% try { SimpleDateFormat formatoFecha = new SimpleDateFormat("dd/MM/yyyy", Locale.getDefault()); formatoFecha.setLenient(false); formatoFecha.parse(request.getParameter("dateInicio"));%><%=request.getParameter("dateInicio").toString().trim()%><% } catch(ParseException  e) {%><%=currentDate%><% } %><%  } else {%><%=currentDate %> <% } %>" title="POP UP" ><i class="icon-signin"></i>No registros desde JDA en Eom</a><br />
