				...
				<div class="well" style="overflow:scroll; height:600px">
				      ...
				      <%
							...
							try{
								...
								SQLFecha = "select ID, SOLICITUDORIGINAL, N_ORDEN_DISTRIBU, SOLICITUD, FECHA_CREACION_ORDEN, ORIGEN, CANCELADO  from CUADRATURA_VENTAV_EOM A  WHERE N_Orden_Distribu NOT IN (SELECT NUMDO FROM CUADRATURA_VENTAV_JDA B WHERE B.NUMDO = A.n_orden_distribu and B.Pofece >= '"+starttime+"' and B.Pofece <= '"+starttime+"') and A.fecha_creacion_orden >= '"+starttime2+" 00:00:00' and A.fecha_creacion_orden <= '"+starttime2+" 23:59:59' ";
//SQLFecha = "select ID, SOLICITUDORIGINAL, N_ORDEN_DISTRIBU, SOLICITUD, FECHA_CREACION_ORDEN, ORIGEN, CANCELADO, CASE WHEN (SELECT CVV_JDA.NUMDO FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '20170418' AND CVV_JDA.Pofece <= '20170418' AND CVV_JDA.NUMDO = A.n_orden_distribu) IS NOT NULL THEN 'Encontrado' ELSE 'No Encontrado' END    as ENCONTRADOENJDA, CASE WHEN (SELECT CVV_JDA.NUMDO FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = A.n_orden_distribu) IS NOT NULL THEN (SELECT CVV_JDA.NUMDO FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = A.n_orden_distribu) ELSE 'Numero solicitud Jda No Encontrado' END    as NUMEROSOLICITUDJDA, CASE WHEN (SELECT CVV_JDA.EX14ERROR FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = A.n_orden_distribu) IS NOT NULL THEN (SELECT CVV_JDA.EX14ERROR FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = A.n_orden_distribu) ELSE 'Mensaje de Error Jda No Encontrado'  END    as MENSAJEERRORJDA, CASE WHEN (SELECT CVV_JDA.Pofece FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = A.n_orden_distribu) IS NOT NULL THEN (SELECT CVV_JDA.Pofece FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = A.n_orden_distribu) ELSE 'Fecha Jda No Encontrado'  END as POFECEJDA  from CUADRATURA_VENTAV_EOM A  WHERE 1 = 1  and A.fecha_creacion_orden >= TO_DATE('"+starttime2+"','DD/MM/YY')||' 00:00:00' and A.fecha_creacion_orden <= TO_DATE('"+starttime2+"','DD/MM/YY')||' 23:59:59' AND A.N_ORDEN_DISTRIBU = '"+request.getParameter("N_Orden_Distribu")+"'";
