				...
				<div class="well" style="overflow:scroll; height:600px">
				      ...
				      <%
							...
							try{
								...
								//SQLFecha = "select ID, PONUMB, POVNUM, NUMDO,POFECE, POHORE,EX14ERROR, EX14FREGI, DESPA   from CUADRATURA_VENTAV_JDA A  WHERE NUMDO NOT IN (SELECT n_orden_distribu FROM CUADRATURA_VENTAV_EOM B WHERe B.n_orden_distribu = A.NUMDO and B.fecha_creacion_orden >= TO_DATE('"+starttime2+"','DD/MM/YY')||' 00:00:00' and B.fecha_creacion_orden <= TO_DATE('"+starttime2+"','DD/MM/YY')||' 23:59:59') and A.Pofece >= '"+starttime+"' and A.Pofece <= '"+starttime+"'";
								SQLFecha = "select ID, PONUMB, POVNUM, NUMDO,POFECE, POHORE,EX14ERROR, EX14FREGI, DESPA   from CUADRATURA_VENTAV_JDA A  WHERE NUMDO NOT IN (SELECT n_orden_distribu FROM CUADRATURA_VENTAV_EOM B WHERe B.n_orden_distribu = A.NUMDO and B.fecha_creacion_orden >= '"+starttime2+" 00:00:00' and B.fecha_creacion_orden <= '"+starttime2+" 23:59:59') and A.Pofece >= '"+starttime+"' and A.Pofece <= '"+starttime+"'";
