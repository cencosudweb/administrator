				...
				<div class="well" style="overflow:scroll; height:600px">
				      ...
				      <%
							...
							try{
								...
								/*SQLFecha = "SELECT  (select CVV_EOM_2.Solicitudoriginal from CUADRATURA_VENTAV_EOM CVV_EOM_2 where CVV_EOM_2.N_Orden_Distribu = CVV_EOM.N_Orden_Distribu) AS Solicitudoriginal, CVV_EOM.N_Orden_Distribu,(select to_char(REPLACE(CVV_EOM_2.Fecha_Creacion_Orden,',000000000',''))  from CUADRATURA_VENTAV_EOM CVV_EOM_2 where CVV_EOM_2.N_Orden_Distribu = CVV_EOM.N_Orden_Distribu) AS FECHA_CREACION_ORDEN, (select CVV_EOM_2.CANCELADO from CUADRATURA_VENTAV_EOM CVV_EOM_2 where CVV_EOM_2.N_Orden_Distribu = CVV_EOM.N_Orden_Distribu) AS CANCELADO, CASE WHEN (SELECT CVV_JDA.NUMDO FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '20170418' AND CVV_JDA.Pofece <= '20170418' AND CVV_JDA.NUMDO = CVV_EOM.n_orden_distribu) IS NOT NULL THEN 'Encontrado' ELSE 'No Encontrado' END    as ENCONTRADOENJDA, CASE WHEN (SELECT CVV_JDA.NUMDO FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = CVV_EOM.n_orden_distribu) IS NOT NULL THEN (SELECT CVV_JDA.NUMDO FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = CVV_EOM.n_orden_distribu) ELSE 'Numero solicitud Jda No Encontrado' END    as NUMEROSOLICITUDJDA, CASE WHEN (SELECT CVV_JDA.EX14ERROR FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = CVV_EOM.n_orden_distribu) IS NOT NULL THEN (SELECT CVV_JDA.EX14ERROR FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = CVV_EOM.n_orden_distribu) ELSE 'Mensaje de Error Jda No Encontrado'  END    as MENSAJEERRORJDA, CASE WHEN (SELECT CVV_JDA.Pofece FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = CVV_EOM.n_orden_distribu) IS NOT NULL THEN (SELECT CVV_JDA.Pofece FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = CVV_EOM.n_orden_distribu) ELSE 'Fecha Jda No Encontrado'  END as POFECEJDA "+
										" from CUADRATURA_VENTAV_EOM CVV_EOM where CVV_EOM.fecha_creacion_orden >= TO_DATE('"+starttime2+"','DD/MM/YY')||' 00:00:00' and CVV_EOM.fecha_creacion_orden <= TO_DATE('"+starttime2+"','DD/MM/YY')||' 23:59:59' " + 
										" group by CVV_EOM.N_Orden_Distribu ORDER BY CVV_EOM.N_Orden_Distribu ASC ";
								*/		
								SQLFecha = "SELECT  (select CVV_EOM_2.Solicitudoriginal from CUADRATURA_VENTAV_EOM CVV_EOM_2 where CVV_EOM_2.N_Orden_Distribu = CVV_EOM.N_Orden_Distribu) AS Solicitudoriginal, CVV_EOM.N_Orden_Distribu,(select to_char(REPLACE(CVV_EOM_2.Fecha_Creacion_Orden,',000000000',''))  from CUADRATURA_VENTAV_EOM CVV_EOM_2 where CVV_EOM_2.N_Orden_Distribu = CVV_EOM.N_Orden_Distribu) AS FECHA_CREACION_ORDEN, (select CVV_EOM_2.CANCELADO from CUADRATURA_VENTAV_EOM CVV_EOM_2 where CVV_EOM_2.N_Orden_Distribu = CVV_EOM.N_Orden_Distribu) AS CANCELADO, CASE WHEN (SELECT CVV_JDA.NUMDO FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '20170418' AND CVV_JDA.Pofece <= '20170418' AND CVV_JDA.NUMDO = CVV_EOM.n_orden_distribu) IS NOT NULL THEN 'Encontrado' ELSE 'No Encontrado' END    as ENCONTRADOENJDA, CASE WHEN (SELECT CVV_JDA.NUMDO FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = CVV_EOM.n_orden_distribu) IS NOT NULL THEN (SELECT CVV_JDA.NUMDO FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = CVV_EOM.n_orden_distribu) ELSE 'Numero solicitud Jda No Encontrado' END    as NUMEROSOLICITUDJDA, CASE WHEN (SELECT CVV_JDA.EX14ERROR FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = CVV_EOM.n_orden_distribu) IS NOT NULL THEN (SELECT CVV_JDA.EX14ERROR FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = CVV_EOM.n_orden_distribu) ELSE 'Mensaje de Error Jda No Encontrado'  END    as MENSAJEERRORJDA, CASE WHEN (SELECT CVV_JDA.Pofece FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = CVV_EOM.n_orden_distribu) IS NOT NULL THEN (SELECT CVV_JDA.Pofece FROM CUADRATURA_VENTAV_JDA CVV_JDA WHERE CVV_JDA.Pofece >= '"+starttime+"' AND CVV_JDA.Pofece <= '"+starttime+"' AND CVV_JDA.NUMDO = CVV_EOM.n_orden_distribu) ELSE 'Fecha Jda No Encontrado'  END as POFECEJDA "+
												" from CUADRATURA_VENTAV_EOM CVV_EOM where CVV_EOM.fecha_creacion_orden >= '"+starttime2+" 00:00:00' and CVV_EOM.fecha_creacion_orden <= '"+starttime2+" 23:59:59' " + 
												" group by CVV_EOM.N_Orden_Distribu ORDER BY CVV_EOM.N_Orden_Distribu ASC ";		
