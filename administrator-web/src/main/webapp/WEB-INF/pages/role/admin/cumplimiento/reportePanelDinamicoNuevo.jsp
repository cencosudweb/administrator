				    ...
				    <table id="dataTables-example"  width="100%"  border="0" cellspacing="0" cellpadding="0" style="font-size:13px">
						<tr bgcolor="#305496" style="text-align:left; color:#FFF; font-size:13px; border: solid 1px #a6a6a6;">
							...
							try{
								...
								/*SQLFecha = 	"	SELECT  " + 
											"		REPLACE (REPLACE (C1.FECHA_COMPROMISO_EOM, '00:', ''), '00','') AS FECHA_COMPROMISO_EOM " +
								...
								*/
								SQLFecha = 	"	SELECT  " + 
											...
											" 		,SUM(CASE  WHEN TC.CUMPLE = '1.- CUMPLE' THEN  1 ELSE 0 END) as CUMPLE " + 
											" 		,SUM(CASE  WHEN TC.CUMPLE = '2.- ATRASADO' THEN  1 ELSE 0 END) as ATRASADO " + 
											" 		,SUM(CASE  WHEN TC.CUMPLE = '3.- NO CUMPLE' THEN  1 ELSE 0 END) as NOCUMPLE " +
											" 		,SUM(CASE  WHEN TC.CUMPLE = '1.- CUMPLE' OR TC.CUMPLE = '2.- ATRASADO' OR TC.CUMPLE = '3.- NO CUMPLE' THEN  1 ELSE 0 END) as TOTAL " +
											//"		,NVL((select count(*) from CUMPLIMIENTO C2 WHERE 1 = 1 AND C2.FECHA_COMPROMISO_EOM = C1.FECHA_COMPROMISO_EOM "+prefixclauseCumple2 + prefixclauseCumplimiento2 + prefixclauseCumplimiento_resumen2 + prefixclauseTipoOrden2 + prefixclauseTipoVenta2 + prefixclauseClienteRetira2 + prefixclauseDescripcionEmpresa2 + prefixclauseTipoGuia2 + prefixclauseHorario2 + prefixclauseVentaEmpresa2+"),0) AS CANTIDAD " + 
						...
						%>
								...
								<tr style="background-color:#8ea9db;color:#FFF; " >
									...
									SQLShowFecha = 	"	SELECT " + 
													...
													" 		,(SUM(CASE WHEN (TC.CUMPLE = '"+rsCumple.getString("CUMPLE")+"' "+prefixclause+") THEN 1 ELSE 0 END) ) AS CANTIDAD " +		
													//"		,NVL( (SUM(CASE  WHEN  (TC.CUMPLE = '"+rsCumple.getString("CUMPLE")+"' "+prefixclause+") THEN  1 ELSE 0 END) ),0) AS CANTIDAD " + 
													"	from CUMPLIMIENTO TC " +
													"	WHERE 1 = 1 " + 
													"	AND TC.FECHA_COMPROMISO_EOM >= REPLACE(TO_CHAR(to_date('"+starttime+"', 'YYYY-MM-DD')-8, 'YYYY/MM/DD'),'/','-') AND TC.FECHA_COMPROMISO_EOM <= '"+starttime+"'  " + 
													//"	" + prefixclause +
													//"	AND TC.CUMPLE = '"+rsCumple.getString("CUMPLE")+"' " +
													"	GROUP BY TC.FECHA_COMPROMISO_EOM " +
													"	ORDER BY TC.FECHA_COMPROMISO_EOM ASC";
								...
								%>
									...
									<tr id="cumplimiento_<%=contarCumple%>_<%=contarCumplimiento%>" style="background-color:#d9e1f2;color:#000000;display:none" >
										...
										SQLShowFecha = 	"	SELECT " + 
														"		TC.FECHA_COMPROMISO_EOM AS  FECHA_COMPROMISO_EOM" +
														//"		,SUM(CASE WHEN TC.CUMPLE_RESUMEN = '"+rsCumplimiento.getString("CUMPLE_RESUMEN")+"' THEN 1 ELSE 0 END) AS CANTIDAD " +
														//" 	,NVL( (SELECT COUNT(*) FROM CUMPLIMIENTO TCS WHERE 1 = 1 AND  TCS.FECHA_COMPROMISO_EOM = TC.FECHA_COMPROMISO_EOM " + "" + "),0) AS  TOTAL" +
														//"		,NVL( (SUM(CASE  WHEN  (TC.CUMPLE_RESUMEN = '"+rsCumplimiento.getString("CUMPLE_RESUMEN")+"' "+prefixclause+") THEN  1 ELSE 0 END) ),0) AS CANTIDAD " + 
														"		,(SUM(CASE WHEN (TC.CUMPLE = '"+rsCumple.getString("CUMPLE")+"' AND TC.CUMPLE_RESUMEN = '"+rsCumplimiento.getString("CUMPLE_RESUMEN")+"' "+prefixclause+") THEN 1 ELSE 0 END) ) AS CANTIDAD " +
														//" 		,NVL( (SELECT COUNT(*) FROM CUMPLIMIENTO TCS WHERE 1 = 1 AND  TCS.FECHA_COMPROMISO_EOM = TC.FECHA_COMPROMISO_EOM " +prefixclauseSub+ "),0) AS  TOTAL" +
														"	from CUMPLIMIENTO TC " +
														"	WHERE 1 = 1 " +
														"	AND TC.FECHA_COMPROMISO_EOM >= REPLACE(TO_CHAR(to_date('"+starttime+"', 'YYYY-MM-DD')-8, 'YYYY/MM/DD'),'/','-') AND TC.FECHA_COMPROMISO_EOM <= '"+starttime+"' " +
														//"	" + prefixclause +
														//" 	AND TC.CUMPLE = '"+rsCumple.getString("CUMPLE")+"' AND TC.CUMPLE_RESUMEN = '"+rsCumplimiento.getString("CUMPLE_RESUMEN")+"' " +
														//" 	AND TC.CUMPLE = '"+rsCumple.getString("CUMPLE")+"' " +
														"	GROUP BY TC.FECHA_COMPROMISO_EOM " + 
														"	ORDER BY TC.FECHA_COMPROMISO_EOM ASC";
										//out.print("<br>");
									...
									SQLCumpleResumen = 	"	SELECT  " + 
														...
														"	AND TC.FECHA_COMPROMISO_EOM >= REPLACE(TO_CHAR(to_date('"+starttime+"', 'YYYY-MM-DD')-8, 'YYYY/MM/DD'),'/','-') AND TC.FECHA_COMPROMISO_EOM <= '"+starttime+"' " + 
														"	AND TC.CUMPLE = '"+rsCumple.getString("CUMPLE")+"' " +
														"	AND TC.CUMPLE_RESUMEN = '"+rsCumplimiento.getString("CUMPLE_RESUMEN")+"'  " + 
														"	GROUP BY TC.CUMPLIMIENTO_2 " +
														"	ORDER BY TC.CUMPLIMIENTO_2 ASC";
									//out.print("<br>");
									...
									%>
										...
										<tr id="cumple_resumen_<%=contarCumple%>_<%=contarCumplimiento%>_<%=contarCumpleResumen%>"  style="background-color:#b7ecff;color:#000000;display:none">
											...
											SQLShowFecha = 	"	SELECT  " +
															...
															//"		,SUM(CASE WHEN TC.CUMPLIMIENTO_2 = '"+rsCumpleResumen.getString("CUMPLIMIENTO_2")+"' THEN 1 ELSE 0 END) AS CANTIDAD " +
															//" 	,NVL( (SELECT COUNT(*) FROM CUMPLIMIENTO TCS WHERE 1 = 1 AND  TCS.FECHA_COMPROMISO_EOM = TC.FECHA_COMPROMISO_EOM " + "" + " ),0) AS  TOTAL" +
															
															//"		,NVL( (SUM(CASE  WHEN  (TC.CUMPLIMIENTO_2 = '"+rsCumpleResumen.getString("CUMPLIMIENTO_2")+"' "+prefixclause+") THEN  1 ELSE 0 END) ),0) AS CANTIDAD " + 
															" 		,(SUM(CASE WHEN (TC.CUMPLE = '"+rsCumple.getString("CUMPLE")+"' AND TC.CUMPLE_RESUMEN = '"+rsCumplimiento.getString("CUMPLE_RESUMEN")+"' AND TC.CUMPLIMIENTO_2 = '"+rsCumpleResumen.getString("CUMPLIMIENTO_2")+"' "+prefixclause+") THEN 1 ELSE 0 END) ) AS CANTIDAD " + 
										...
										%>
											<tr id="responsable_<%=contarCumple%>_<%=contarCumplimiento%>_<%=contarCumpleResumen%>_<%=contarResponsable%>" style="display:none">
												...
												SQLShowFecha =	"	SELECT  " +
																"		TC.FECHA_COMPROMISO_EOM " +
																//"		,SUM(CASE WHEN TC.RESPONSABLE = '"+rsResponsable.getString("RESPONSABLE")+"' THEN 1 ELSE 0 END) AS CANTIDAD " +
																//" 		,NVL( (SELECT COUNT(*) FROM CUMPLIMIENTO TCS WHERE 1 = 1 AND  TCS.FECHA_COMPROMISO_EOM = TC.FECHA_COMPROMISO_EOM " + "" + "),0) AS  TOTAL" +
																
																//"		,NVL( (SUM(CASE  WHEN  (TC.RESPONSABLE = '"+rsResponsable.getString("RESPONSABLE")+"' "+prefixclause+") THEN  1 ELSE 0 END) ),0) AS CANTIDAD " + 
																//" 		,NVL( (SELECT COUNT(*) FROM CUMPLIMIENTO TCS WHERE 1 = 1 AND  TCS.FECHA_COMPROMISO_EOM = TC.FECHA_COMPROMISO_EOM " +prefixclauseSub+ "),0) AS  TOTAL" +
																" 		,(SUM(CASE WHEN (TC.CUMPLE = '"+rsCumple.getString("CUMPLE")+"' AND TC.CUMPLIMIENTO_2 = '"+rsCumpleResumen.getString("CUMPLIMIENTO_2")+"' AND TC.RESPONSABLE = '"+rsResponsable.getString("RESPONSABLE")+"' "+prefixclause+") THEN 1 ELSE 0 END) ) AS CANTIDAD " + 
																...
																//" 	AND TC.CUMPLE = '"+rsCumple.getString("CUMPLE")+"' " +
																//"	AND TC.CUMPLE_RESUMEN = '"+rsCumplimiento.getString("CUMPLE_RESUMEN")+"' " +
																//"	AND TC.CUMPLIMIENTO_2 = '"+rsCumpleResumen.getString("CUMPLIMIENTO_2")+"' " +
																"	GROUP BY TC.FECHA_COMPROMISO_EOM " +
																"	ORDER BY TC.FECHA_COMPROMISO_EOM ASC";
												//out.print("<br>");
