	        ...
	        <div class="row-fluid">
				...
				<div class="btn-toolbar">
					...
					%>
						...
						<a target="_blank" class="btn" href="reportePanelDinamicoPrint?exportToExcel=YES&posted=<%if(request.getParameter("posted") != null){%><%=request.getParameter("posted")%><%}else{%><%=""%><% } %>&dateInicio=<% if (request.getParameter("dateInicio") != null) { %><% try { SimpleDateFormat formatoFecha = new SimpleDateFormat("dd/MM/yyyy", Locale.getDefault()); formatoFecha.setLenient(false); formatoFecha.parse(request.getParameter("dateInicio"));%><%=request.getParameter("dateInicio").toString().trim()%><% } catch(ParseException  e) {%><%=currentDate%><% } %><%  } else {%><%=currentDate %> <% } %>&tipo_orden=<%if(request.getParameter("tipo_orden") != null){%><%=request.getParameter("tipo_orden")%><%}else{%><%="NULL"%><% } %>&tipo_venta=<%if(request.getParameter("tipo_venta") != null){%><%=request.getParameter("tipo_venta")%><%}else{%><%="NULL"%><% } %>&cliente_retira=<%if(request.getParameter("cliente_retira") != null){%><%=request.getParameter("cliente_retira")%><%}else{%><%="NULL"%><% } %>&descripcion_empresa=<%if(request.getParameter("descripcion_empresa") != null){%><%=request.getParameter("descripcion_empresa")%><%}else{%><%="NULL"%><% } %>&tipo_guia=<%if(request.getParameter("tipo_guia") != null){%><%=request.getParameter("tipo_guia")%><%}else{%><%="NULL"%><% } %>&tipo_horario=<%if(request.getParameter("tipo_horario") != null){%><%=request.getParameter("tipo_horario")%><%}else{%><%="NULL"%><% } %>&venta_empresa=<%if(request.getParameter("venta_empresa") != null){%><%=request.getParameter("venta_empresa")%><%}else{%><%="NULL"%><% } %>&tipo_estado=<%if(request.getParameter("tipo_estado") != null){%><%=request.getParameter("tipo_estado")%><%}else{%><%="NULL"%><% } %>"><i class="icon-book"></i> Exportar a Excel</a>
						
					<%
						} 
					%>
					<a class="btn" href="downloadExcelCumplimiento?dateInicio=<%=dateInicio%>"><i class="icon-book"></i> Export Excel BD</a>
